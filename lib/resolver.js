...
function resolve(config, modulePath, deps) {
	...
	resolvedDeps = deps.map(function(depPath) {
		...
		if (/^\.{1,2}\//.test(depPath)) {
			...
			resolvedPath = path.join(path.dirname(modulePath), depPath);
		} else {
			// module name
			...
			if (moduleConfig) {
				...
				resolvedPath = path.join(baseUrl, moduleConfig.location, subPath ? subPath : moduleConfig.main || "main");
			} else {
				// 2nd: not found by modules, try path

				if (config.paths[moduleName]) {
					// found by path
					resolvedPath = path.join(baseUrl, config.paths[moduleName], subPath);
				} else {
					// 3rd: even no module config, try looking up at baseUrl
					resolvedPath = path.join(baseUrl, depPath);
				}
			}
		}
